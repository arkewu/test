...
abstract class EbayApiAbstract implements PlatformApiInterface {
    ...
    public function setAccount($accountID){
        ...
        $tokenInfo = PlatformCommonAccount::getTokenInfoByIDAndPlatform($this->accountID, Platform::CODE_EBAY);
        $ebayKeys = ConfigFactory::getConfig('ebayKeys');
        $this->accountID    = $accountID;
        $this->_usertoken   = $tokenInfo['user_token'];
        $this->_appID       = $tokenInfo['appid'];
        $this->_devID       = $tokenInfo['devid'];
        $this->_certID      = $tokenInfo['certid'];
