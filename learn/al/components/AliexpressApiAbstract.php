...
abstract class AliexpressApiAbstract implements PlatformApiInterface {
    ...
    public function setAccount($accountID,$code=null){
        ...
        //获取账号相关信息
        // $accountInfo = AliexpressAccount::getAccountInfoById($this->accountID);
        $tokenInfo = PlatformCommonAccount::getTokenInfoByIDAndPlatform($this->accountID, Platform::CODE_ALIEXPRESS);
        $aliexpressKeys = ConfigFactory::getConfig('aliexpressKeys');
        $this->accountID        = $accountID;
        $this->_access_token    = $tokenInfo['access_token'];
        $this->_refresh_token   = $tokenInfo['refresh_token'];
        $this->_appKey          = $tokenInfo['app_key'];
        $this->_secretKey       = $tokenInfo['secret_key'];
