<?php
/**
 * @desc Wish API Abstract
 * @author Gordon
 * @since 2015-06-22
 */
abstract class WishApiAbstract implements PlatformApiInterface {
    ...
    public function setAccount($accountID){
        $tokenInfo                  = PlatformCommonAccount::getTokenInfoByIDAndPlatform($accountID, Platform::CODE_WISH);//获取token相关信息
        $wishKeys                   = ConfigFactory::getConfig('wishKeys');
        $this->accountID            = $accountID;
        $this->_accessToken         = $tokenInfo['access_token'];
        $this->_refreshToken        = $tokenInfo['refresh_token'];
        $this->_clientID            = $tokenInfo['client_id'];
        $this->_clientSecret        = $tokenInfo['client_secret'];
        $this->_tokenExpiredTime    = strtotime($tokenInfo['token_invalid_time']);
    ...
    public function refreshToken() {
    	...
    	if ($refreshTokenRequest->getIfSuccess()) {
    		...
    		WishAccount::model()->updateByPk($this->accountID, $data);
    	}
    }
